...
export class BalanceComponent implements OnInit {
  ...
  private loadBalances() {
    ...
    forkJoin({
      balances: this.auth
        .getBalancesForGroup(this.accountId)
        .pipe(catchError(() => of([]))),
      detailed: this.auth
        .getDetailedBalancesForGroup(this.accountId)
        .pipe(catchError(() => of([]))),
    }).subscribe({
      next: ({ balances, detailed }) => {
        ...
        if (me) {
          const meDet = this.detailedBalances.find((d) => {
            const id =
              d?.userId || d?.user?._id || d?.user?.id || d?.user?.userId;
      ...
      error: (err: any) => {
        ...
        this.error =
          err?.error?.message ||
          err?.message ||
          'No se pudieron cargar balances';
      },
    });
  }
  ...
  requestPayment(ow: any) {
    ...
    const debtorName =
      ow?.userName || ow?.nombre || ow?.email || ow?.userEmail || debtorId;
    ...
    this.notificationService.createNotification(notification).subscribe({
      next: () => {
        alert(
          `Se ha enviado una solicitud de pago a ${debtorName} por ${amount.toFixed(2)}`,
        );
        this.requestingKeys.delete(key);
      },
      error: (err: any) => {
