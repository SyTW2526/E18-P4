...
@Component({
  ...
  template: `
    <section
      style="max-width:800px;margin:0 auto;padding:1.5rem;min-height:calc(100vh - 120px)"
    >
      <div
      ...
      <div
        *ngIf="error"
        style="padding:1rem;background:#f443;border-radius:6px;color:crimson;margin-bottom:1rem"
      >
        {{ error }}
      </div>
      ...
      >
        <mat-card
          *ngFor="let notif of notifications"
          ...
          [style.border-left]="
            notif.leida ? '4px solid #555' : '4px solid #22c55e'
          "
        >
          <div style="display:flex;gap:1rem;align-items:flex-start">
            <!-- Icono según tipo -->
            <mat-icon
              [style.color]="getIconColor(notif.tipo)"
            ...
            <div style="flex:1">
              ...
              <div
                style="font-weight:600;font-size:1rem;color:#e0e0e0;margin-bottom:0.5rem"
              >
                {{ getTitle(notif) }}
              </div>
              ...
              <!-- Botones de acción -->
              <div
                *ngIf="notif.tipo === 'solicitud_pago' && !notif.respondida"
              ...
              >
                ...
                <button
                  mat-button
                  (click)="rejectPaymentRequest(notif)"
                  [disabled]="processing === notif._id"
                >
              ...
              >
                ✓ {{ lang.t('responded') || 'Respondida' }}
              </div>

              <div
                *ngIf="notif.tipo === 'grupo_invitacion' && !notif.respondida"
                style="display:flex;gap:0.5rem;margin-top:0.75rem"
              >
                <button
                  mat-raised-button
              ...
              <div
                *ngIf="notif.tipo === 'solicitud_amistad' && !notif.respondida"
                style="display:flex;gap:0.5rem;margin-top:0.75rem"
              >
                <button
                  mat-raised-button
                ...
                <button
                  mat-button
                  (click)="rejectFriendRequest(notif)"
                  [disabled]="processing === notif._id"
                >
                  <mat-icon>close</mat-icon>
                  {{ lang.t('reject') || 'Rechazar' }}
...
export class NotificationsComponent implements OnInit, OnDestroy {
  ...
  loadNotifications() {
    ...
    forkJoin({
      ...
      next: ({ notifications, invitations }) => {
        const mappedInvites: Notification[] = (invitations || []).map(
          (inv: any) => ({
            _id: inv._id,
            ...
            mensaje:
              inv.mensaje ||
              `Invitación al grupo ${inv.grupo?.nombre || ''}`.trim(),
            leida: false,
            respondida: !!inv.respondida,
            fecha: inv.fecha_invitacion
              ? new Date(inv.fecha_invitacion)
              : new Date(),
        ...
        this.notifications = [...mappedInvites, ...notifications].sort(
          (a, b) => {
  ...
  getMessage(notif: Notification): string {
    ...
    switch (notif.tipo) {
      case 'solicitud_pago':
        return (
          (this.lang.t('paymentRequestMessage') || 'Te solicitan el pago de ') +
          `${notif.monto?.toFixed(2) || '0.00'}`
        );
      case 'pago_confirmado':
        return (
          (this.lang.t('paymentConfirmedMessage') || 'Pago de ') +
          `${notif.monto?.toFixed(2) || '0.00'} ` +
          (this.lang.t('confirmed') || 'confirmado')
        );
      case 'gasto_creado':
        return (
          this.lang.t('newExpenseMessage') ||
          'Se ha creado un nuevo gasto en el grupo'
        );
      case 'grupo_invitacion':
        return (
          this.lang.t('groupInvitationMessage') ||
          'Te han invitado a unirte a un grupo'
        );
      case 'solicitud_amistad':
        return (
          this.lang.t('friendRequestMessage') ||
  ...
  formatDate(fecha: Date): string {
    ...
    if (minutes < 1) return this.lang.t('nowText') || 'Ahora';
    if (minutes < 60) return `${this.lang.t('ago') || 'Hace'} ${minutes} min`;
    if (hours < 24) return `${this.lang.t('ago') || 'Hace'} ${hours}h`;
    if (days < 7) return `${this.lang.t('ago') || 'Hace'} ${days}d`;
  ...
  acceptPaymentRequest(notif: Notification) {
    ...
    this.notificationService.markAsResponded(notif._id).subscribe({
      next: () => {
  ...
  acceptInvitation(notif: Notification) {
    ...
    this.processing = notif._id;
    this.authService
      .acceptGroupInvitation(notif._id, String(userId))
      .subscribe({
        next: () => {
  ...
  rejectInvitation(notif: Notification) {
    ...
    this.processing = notif._id;
    this.authService
      .rejectGroupInvitation(notif._id, String(userId))
      .subscribe({
        next: () => {
