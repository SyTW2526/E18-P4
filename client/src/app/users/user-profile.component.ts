    ...
    <div style="max-width:1000px; margin:88px auto 24px; padding:18px; background:var(--secondary-bg); border-radius:8px; color:var(--text-main);">
      ...
      <ng-container *ngIf="notAllowed">
        <div style="padding:24px; text-align:center;">
          <h2 style="margin:0 0 8px; color:var(--primary-color)">{{ targetDisplayName || 'Username' }}</h2>
...
export class UserProfileComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.auth.getUserById(id).subscribe({
      next: (u) => { this.targetDisplayName = u?.nombre || u?.username || u?.email || ''; },
      error: (err) => { 
    ...
    this.auth.getAmigos(String(myId)).subscribe({
      next: (res: any) => {
        const friendsData: any[] = (res && res.amigos) ? res.amigos : (Array.isArray(res) ? res : []);
        ...
        if (!found) {
          ...
          if (friendObj && (friendObj.nombre || friendObj.email)) {
            this.targetDisplayName = friendObj.nombre || friendObj.username || friendObj.email || '';
          }
          return;
        }
  ...
  private loadProfile(id: string) {
    this.auth.getUserById(id).subscribe({
      ...
      error: (err) => { 
        ...
        this.notAllowed = true;
        this.requestMessage = err?.error?.message || 'No se pudo cargar el perfil';
      }
    });
  }
